...
def discover_images(input_dir: Path) -> list[Path]:
    ...
    if len(images) < 2:
        raise RuntimeError(
            f"입력 이미지가 {len(images)}장뿐입니다. "
            f"{input_dir} 폴더에 2장 이상의 이미지를 넣으세요."
        )
    return images
...
def read_image(path: Path) -> np.ndarray:
    ...
    if image.ndim == 3 and image.shape[2] in (3, 4):
        ...
        return image
    raise RuntimeError(f"지원하지 않는 이미지 배열입니다: {path} / shape={image.shape}")
...
def compute_features(images: list[np.ndarray], max_features: int) -> list[FeatureData]:
    ...
    for idx, image in enumerate(images, start=1):
        ...
        print(f"  - 이미지 {idx:02d}: {count} keypoints")
    return features
...
def pairwise_matches(
    ...
    print(f"\n[2/6] 모든 이미지 쌍의 SIFT 매칭 및 RANSAC Homography 계산 ({total}쌍)")
    for i in range(n):
...
def maximum_spanning_tree(n: int, matches: list[PairMatch]) -> list[PairMatch]:
    ...
    if len(tree) != n - 1:
        ...
        raise RuntimeError(
            "모든 이미지가 하나의 연결망으로 이어지지 않았습니다. "
            f"현재 연결 그룹: {comp_text}. 인접 이미지의 겹침을 늘리거나 입력 이미지를 확인하세요."
        )
    return tree
...
def compose_global_transforms(
    ...
    if any(H is None for H in transforms):
        raise RuntimeError("전역 Homography 구성에 실패했습니다.")
...
def build_canvas(
    ...
    if width <= 0 or height <= 0:
        raise RuntimeError("유효한 출력 캔버스를 계산하지 못했습니다.")
    if width * height > 2_000_000_000:
        raise RuntimeError(
            f"예상 캔버스가 지나치게 큽니다: {width} x {height}. "
            "잘못된 특징점 연결이 있을 수 있습니다."
        )
...
def longest_contiguous_run(values: np.ndarray) -> tuple[int, int]:
    if len(values) == 0:
        raise RuntimeError("겹침 영역이 없습니다.")
...
def minimum_error_horizontal_seam(
    ...
    *,
    ...
    if len(xs) == 0:
        raise RuntimeError("두 영상 사이에 겹침 영역이 없습니다.")
    ...
    if len(possible_last) == 0:
        raise RuntimeError("최소 오차 seam 계산에 실패했습니다.")
    ...
    for x in range(width - 1, 0, -1):
        ...
        if y < 0:
            raise RuntimeError("seam 역추적에 실패했습니다.")
        local_seam[x - 1] = y
...
def save_tiff(path: Path, image: np.ndarray) -> None:
    ...
    if not ok:
        raise RuntimeError(f"TIFF 저장에 실패했습니다: {path}")
...
def mosaic_blend(
    ...
    for step, idx in enumerate(order[1:], start=1):
        ...
        if not np.any(overlap):
            raise RuntimeError(
                f"이미지 {idx+1}은 현재 합성본과 겹치지 않습니다. "
                "전역 정렬 연결을 확인하세요."
            )
        ...
        print(
            f"  - [{step}/{len(order)-1}] 이미지 {idx+1:02d} 추가 / "
            f"기준 이미지 {nearest+1:02d} / {layout} seam"
        )
...
def main() -> None:
    ...
    for path, image in zip(paths, images):
        if image.dtype != dtype0:
            raise RuntimeError(
                f"모든 입력 이미지의 비트 심도/자료형이 같아야 합니다. "
                f"기준={dtype0}, {path.name}={image.dtype}"
            )
        if (image.ndim == 2) != (images[0].ndim == 2):
            raise RuntimeError("그레이스케일 영상과 컬러 영상을 혼합할 수 없습니다.")
        if image.ndim == 3 and image.shape[2] != images[0].shape[2]:
            raise RuntimeError("컬러 채널 수가 서로 다른 영상을 혼합할 수 없습니다.")

    features = compute_features(images, args.max_features)
    matches = pairwise_matches(images, features, args)
    print("\n[3/6] RANSAC 연결망에서 최대 신장 트리 구성")
    ...
    print(f"  - 자동 선택 기준 이미지: {reference+1:02d} ({paths[reference].name})")
    print(f"  - 합성 순서: {', '.join(str(i+1) for i in order)}")
    ...
    print("합성 완료")
    print("===============================================")
    print(f"1. seam 표시본 : {preview_path}")
    print(f"2. cropping 전 : {full_path}")
    print(f"3. cropping 후 : {cropped_path}")
    print("\n※ 논문용 결과는 SEM_stitched_cropped.tif를 권장합니다.")
